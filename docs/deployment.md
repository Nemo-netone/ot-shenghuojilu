...
| 稳定演示地址 | `https://ot-shenghuojilu.pages.dev` |
...
该脚本只创建和写入 `ot_shenghuojilu`，不会创建、删除、重置或覆盖 `public` schema 及其他项目 schema。

## 发布命令

```powershell
supabase db query --linked --file supabase/schema.sql
npx wrangler@3 pages deploy site --project-name ot-shenghuojilu --branch main
```
